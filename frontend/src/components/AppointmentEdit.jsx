...
import { editAppointmentApi, getAvailableSlotsApi, viewAppointmentApi } from '../services/Api'
...
function AppointmentEdit() {
    const { id } = useParams();
    useEffect(() => {
        viewAppointmentApi(id)
            .then(res => {
                setAppointment(res.data);
                console.log(res.data.time_slot)
                return getAvailableSlotsApi(res.data.date, id);  // <- fetch slots after getting date
            })
            .then(res => {
    ...
    const [appointment, setAppointment] = useState({ name: "", phone_number: "", date: "", time_slot: "" })
    ...
    async function handleSubmit(event) {
        ...
        let response = await editAppointmentApi(id, appointment)
    ...
    async function handleDateChange(event) {
        ...
        let response = await getAvailableSlotsApi(selectedDate, id)
        if (response.status >= 200 && response.status < 300) {
